...
/* ******************************************
 *                                          *
 *      ORIGINAL TESTS FROM ICO PROJECT     *
 *        Withdraw is added at the top      *
 *     check for WITHDRAW LP TEST comment   *
 *                                          *
 ********************************************/

describe("ICO Withdraw", () => {
  async function setupFixture() {
    ...
    const spaceCoinIco: SpaceCoinIco = (await SpaceCoinIco.deploy(
      spaceCoin.address,
      treasury.address,
  ...
  it("Allow to withdraw all funds", async () => {
    const { spaceCoinIco, nonWhitelistedInvestor, treasury } = await loadFixture(setupFixture);

    await spaceCoinIco.phaseAdvance(IcoPhase.SEED);
    await spaceCoinIco.phaseAdvance(IcoPhase.GENERAL);
    await spaceCoinIco.connect(nonWhitelistedInvestor).contribute({ value: ONE_ETHER.mul(30_000) });

    expect(await spaceCoinIco.amountRaised()).to.be.equal(ONE_ETHER.mul(30_000));
    await spaceCoinIco.withdraw(ONE_ETHER.mul(30_000));
    expect(await ethers.provider.getBalance(spaceCoinIco.address)).to.be.equal(0);

    // treasury starts at 50k eth (hardhat configuration)
    // so it has to be 30k = 50k + 80k
    expect(await treasury.getBalance()).to.be.closeTo(ONE_ETHER.mul(80_000), ethers.utils.parseEther("0.1"));
  });

  it("Allow to withdraw funds in any phase and in chunks", async () => {
    const { spaceCoinIco, whitelistedAddresses, treasury } = await loadFixture(setupFixture);
    const investor = whitelistedAddresses[0];
    expect(await spaceCoinIco.currentPhase()).to.be.equal(IcoPhase.SEED);
    await spaceCoinIco.connect(investor).contribute({ value: ONE_ETHER.mul(1_000) });

    expect(await spaceCoinIco.amountRaised()).to.be.equal(ONE_ETHER.mul(1_000));
    await spaceCoinIco.withdraw(ONE_ETHER.mul(500));
    expect(await ethers.provider.getBalance(spaceCoinIco.address)).to.be.equal(ONE_ETHER.mul(500));
    expect(await treasury.getBalance()).to.be.closeTo(ONE_ETHER.mul(50_500), ethers.utils.parseEther("0.1"));

    await spaceCoinIco.phaseAdvance(IcoPhase.SEED);

    expect(await spaceCoinIco.currentPhase()).to.be.equal(IcoPhase.GENERAL);
    expect(await spaceCoinIco.amountRaised()).to.be.equal(ONE_ETHER.mul(1_000));
    await spaceCoinIco.withdraw(ONE_ETHER.mul(250));
    expect(await ethers.provider.getBalance(spaceCoinIco.address)).to.be.equal(ONE_ETHER.mul(250));
    expect(await treasury.getBalance()).to.be.closeTo(ONE_ETHER.mul(50_750), ethers.utils.parseEther("0.1"));

    await spaceCoinIco.phaseAdvance(IcoPhase.GENERAL);

    expect(await spaceCoinIco.currentPhase()).to.be.equal(IcoPhase.OPEN);
    expect(await spaceCoinIco.amountRaised()).to.be.equal(ONE_ETHER.mul(1_000));
    await spaceCoinIco.withdraw(ONE_ETHER.mul(250));
    expect(await ethers.provider.getBalance(spaceCoinIco.address)).to.be.equal(0);
    expect(await treasury.getBalance()).to.be.closeTo(ONE_ETHER.mul(51_000), ethers.utils.parseEther("0.1"));

    await spaceCoinIco.connect(investor).contribute({ value: ONE_ETHER.mul(29_000) });
    expect(await spaceCoinIco.amountRaised()).to.be.equal(ONE_ETHER.mul(30_000));

    await spaceCoinIco.withdraw(ONE_ETHER.mul(9_000));
    expect(await ethers.provider.getBalance(spaceCoinIco.address)).to.be.equal(ONE_ETHER.mul(20_000));
    expect(await treasury.getBalance()).to.be.closeTo(ONE_ETHER.mul(60_000), ethers.utils.parseEther("0.1"));

    await spaceCoinIco.withdraw(ONE_ETHER.mul(20_000));
    expect(await ethers.provider.getBalance(spaceCoinIco.address)).to.be.equal(0);
    expect(await treasury.getBalance()).to.be.closeTo(ONE_ETHER.mul(80_000), ethers.utils.parseEther("0.1"));

    await expect(spaceCoinIco.connect(investor).contribute({ value: ONE_ETHER })).to.be.revertedWithCustomError(
      spaceCoinIco,
      "MustBeUnderLimit"
    );

    await expect(spaceCoinIco.withdraw(ONE_ETHER.mul(10_000))).to.be.revertedWithCustomError(
      spaceCoinIco,
      "NotEnoughBalance"
    );
  });
  it("Block withdraw if it's not the owner", async () => {
    const { spaceCoinIco, nonWhitelistedInvestor } = await loadFixture(setupFixture);
    await expect(spaceCoinIco.connect(nonWhitelistedInvestor).withdraw(ONE_ETHER.mul(30_000)))
      .to.be.revertedWithCustomError(spaceCoinIco, "MustBeOwner")
      .withArgs("NOT_OWNER");
  });
  it("Block withdraw if amount raised it 0", async () => {
    const { spaceCoinIco, nonWhitelistedInvestor, treasury } = await loadFixture(setupFixture);
    await expect(spaceCoinIco.withdraw(ONE_ETHER.mul(30_000))).to.be.revertedWithCustomError(
      spaceCoinIco,
      "AmountRaisedEqualZero"
    );
  });
  it("Block withdraw if amount requested is more than the contract balance", async () => {
    const { spaceCoinIco, nonWhitelistedInvestor, treasury } = await loadFixture(setupFixture);
    await spaceCoinIco.phaseAdvance(IcoPhase.SEED);
    await spaceCoinIco.phaseAdvance(IcoPhase.GENERAL);
    await spaceCoinIco.connect(nonWhitelistedInvestor).contribute({ value: ONE_ETHER.mul(10_000) });
    await expect(spaceCoinIco.withdraw(ONE_ETHER.mul(30_000))).to.be.revertedWithCustomError(
      spaceCoinIco,
      "NotEnoughBalance"
    );
...
describe("SpaceCoin", () => {
  ...
  describe("Token Contract", () => {
    ...
    it("Allocates 150000 SPC of supply for the ICO", async () => {
      const { spaceCoin, spaceCoinIco, deployer } = await loadFixture(setupFixture);
      expect(await spaceCoin.balanceOf(spaceCoinIco.address)).to.be.equal(ICO_SUPPLY);
      expect(await spaceCoin.balanceOf(deployer.address)).to.be.equal(0);
      expect(await spaceCoin.totalSupply()).to.be.equal(TOTAL_SUPPLY);
    });
  ...
  describe("SpaceCoinIco", () => {
    ...
    describe("Get Owner", () => {
      it("Owner is deployer", async () => {
        const { spaceCoinIco, deployer } = await loadFixture(setupFixture);
        expect(await spaceCoinIco.getOwner()).to.be.equal(deployer.address);
      });
    });
    ...
    describe("Phase Management", () => {
      ...
      it("Prevent from advancing if phase is wrong", async () => {
        const { spaceCoinIco, whitelistedAddresses, nonWhitelistedInvestor } = await loadFixture(setupFixture);
        const [alice] = whitelistedAddresses;
        expect(await spaceCoinIco.currentPhase()).to.be.equal(0);
    ...
    describe("Pause/Resume", () => {
      it("Allow onwer to pause and resume the ICO", async () => {
        ...
        await spaceCoinIco.setPaused(true);
        expect(await spaceCoinIco.paused()).to.be.true;
        await spaceCoinIco.setPaused(false);
        expect(await spaceCoinIco.paused()).to.be.false;
      });

      it("Prevent onwer to pause/resume the ICO if it's in the same state", async () => {
        const { spaceCoinIco } = await loadFixture(setupFixture);
        expect(await spaceCoinIco.paused()).to.be.false;
        await expect(spaceCoinIco.setPaused(false)).to.be.revertedWithCustomError(
          spaceCoinIco,
          "MustHaveDifferentPauseState"
        );

        await spaceCoinIco.setPaused(true);
        await expect(spaceCoinIco.setPaused(true)).to.be.revertedWithCustomError(
          spaceCoinIco,
          "MustHaveDifferentPauseState"
        );
      });
      ...
      it("Prevent non owner to pause or resume the ICO", async () => {
        ...
        await expect(spaceCoinIco.connect(alice).setPaused(true))
          .to.be.revertedWithCustomError(spaceCoinIco, "MustBeOwner")
          .withArgs("NOT_OWNER");
        await expect(spaceCoinIco.connect(nonWhitelistedInvestor).setPaused(false))
          .to.be.revertedWithCustomError(spaceCoinIco, "MustBeOwner")
          .withArgs("NOT_OWNER");
      ...
      it('Emit "Pause" event when setting Pause/Resume', async () => {
        const { spaceCoinIco } = await loadFixture(setupFixture);
        expect(await spaceCoinIco.paused()).to.be.false;
        await expect(spaceCoinIco.setPaused(true)).to.emit(spaceCoinIco, "Paused").withArgs(true);
        expect(await spaceCoinIco.paused()).to.be.true;
        await spaceCoinIco.setPaused(false);
    ...
    describe("Funding and Tokens Redemption", () => {
      describe("Seed Phase", () => {
        ...
        it("Allows contributors to earn tokens", async () => {
          ...
          const [alice] = whitelistedAddresses;
          await spaceCoinIco.connect(alice).contribute({ value: ethers.utils.parseEther("10") });
          expect(await spaceCoinIco.connect(alice).addressToTokenDistribute(alice.address)).to.be.equal(
